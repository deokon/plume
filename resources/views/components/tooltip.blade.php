...
@php
    ...
    $arrowClasses = match($position) {
        ...
        // 'top'
        default => 'top-full left-1/2 -translate-x-1/2 border-t-background-800 dark:border-t-background-200 border-x-transparent border-b-transparent',
    };
@endphp
...
<div {{ $attributes->merge(['class' => 'relative group inline-block']) }}>
    ...
    <div class="absolute z-50 invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-opacity duration-200 {{ $positionClasses }}">
        <div class="relative bg-background-800 text-background-200 dark:bg-background-200 dark:text-background-800 text-xs rounded py-1 px-2 whitespace-nowrap shadow-md">
            {{ $text }}
