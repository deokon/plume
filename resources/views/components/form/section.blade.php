...
<fieldset {{ $attributes->merge(['class' => 'grid gap-x-6 gap-y-4 pb-4 border-b items-start border-background-700/40 dark:border-background-400/20 ' . $cols]) }}>
    @if($title || $description)
        <div class="space-y-1 mb-2 col-span-full">
            @if($title)
                <h3 class="text-lg font-medium text-foreground dark:text-background-200">{{ $title }}</h3>
            @endif
            @if($description)
