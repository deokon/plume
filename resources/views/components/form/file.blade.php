...
<x-plume::form.element :label="$label ?? $slot" :name="$name" :id="$id" :model="$model">
    ...
    >
        <div class="space-y-1 text-center">
            <x-plume::icon i="icon-[fluent--image-add-24-regular]" class="mx-auto h-12 w-12 text-foreground/30 dark:text-background-400" />
            <div class="flex text-sm">
                <label for="{{ $id }}" class="relative cursor-pointer font-medium text-primary hover:text-primary-500 focus-within:outline-none">
                    <span>Upload a file</span>
                    <input id="{{ $id }}" name="{{ $name }}" type="file" class="sr-only" x-ref="input" @change="handleFileSelect">
                </label>
            ...
            <template x-if="file">
                <div class="pt-4">
                    ...
                    <button type="button" @click="removeFile" class="text-sm text-destructive hover:text-destructive-800">Remove</button>
                </div>
            </template>
        </div>
    </div>
